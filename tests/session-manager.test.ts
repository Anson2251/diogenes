...
describe("ACP session lifecycle", () => {
    ...
    it("updates session title and description when task.end includes metadata", async () => {
        ...
        expect(
            notifications.some(
                (item) => item.params?.update?.sessionUpdate === "session_info_update",
            ),
        ).toBe(true);
    });
    ...
    it("disposes idle sessions and clears registered resources", async () => {
        ...
        await expect(session.prompt([{ type: "text", text: "again" }])).rejects.toThrow(
            "Session is disposed",
        );
    });
...
describe("SessionManager lifecycle", () => {
    ...
    it("prunes broken persisted session directories", async () => {
        ...
        try {
            ...
            expect(result.reasonsBySessionId).toEqual(
                expect.objectContaining({
...
describe("ACPServer disposal", () => {
    ...
    it("lists and closes live sessions through ACP management methods", async () => {
        ...
        try {
            ...
            expect(
                disposeResponse && "result" in disposeResponse ? disposeResponse.result : null,
            ).toEqual({
                disposed: true,
                sessionId,
            });
