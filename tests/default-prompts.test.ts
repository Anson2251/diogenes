...
describe("default prompts", () => {
    it("keeps the Diogenes tool protocol and task completion requirements", () => {
        ...
        expect(DEFAULT_SYSTEM_PROMPT).toContain("include `task.end` in the final `tool-call` block");
    });
    ...
    it("includes upgraded intent, efficiency, and safety guidance", () => {
        ...
        expect(DEFAULT_SYSTEM_PROMPT).toContain("stage, commit, or revert changes unless the user explicitly asks");
        expect(DEFAULT_SYSTEM_PROMPT).toContain("expose, print, or store secrets");
        expect(DEFAULT_SYSTEM_PROMPT).toContain("the final `task.end.summary` should contain the substantive answer itself");
    });
