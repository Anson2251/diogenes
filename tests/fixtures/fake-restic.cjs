...
async function main() {
  ...
  if (subcommand === "restore") {
    ...
    if (restoreRoot) {
      ...
      fs.writeFileSync(
        path.join(restoreRoot, "hello.txt"),
        process.env.FAKE_RESTIC_RESTORE_HELLO || "restored\n",
      );
      fs.mkdirSync(path.join(restoreRoot, "nested"), { recursive: true });
      fs.writeFileSync(
        path.join(restoreRoot, "nested", "data.json"),
        process.env.FAKE_RESTIC_RESTORE_JSON || '{"restored":true}\n',
      );
    }
