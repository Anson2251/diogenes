...
function loadConfig(configPath: string): Partial<DiogenesConfig> {
  try {
    ...
    if (ext === '.json') {
      ...
      // Note: yaml package is already a dependency
      const yaml = require('yaml');
      return yaml.parse(configContent);
    } else {
      console.error(`${colors.yellow}Warning: Unsupported config file format ${ext}, using JSON${colors.reset}`);
      return JSON.parse(configContent);
    }
  } catch (error) {
    console.error(`${colors.red}Error loading config file ${configPath}:${colors.reset}`, error);
    process.exit(1);
  }
}
  ...
  try {
    const result = await executeTask(taskDescription, config, {
      ...
      onToolCall: (toolCalls) => {
        console.log(`${colors.yellow}${colors.bright}Tool Calls (${toolCalls.length}):${colors.reset}`);
        for (let i = 0; i < toolCalls.length; i++) {
          const toolCall = toolCalls[i];
          console.log(`${colors.yellow}[${i + 1}] ${toolCall.tool}:${colors.reset}`);
          console.log(JSON.stringify(toolCall.params, null, 2));
          console.log();
        }
      },
      ...
      onError: (error) => {
        ...
        if (errorMsg.includes('network') || errorMsg.includes('fetch') || errorMsg.includes('failed')) {
          ...
          console.error(`2. Verify the API endpoint: ${config.llm?.baseURL || 'https://api.openai.com/v1'}`);
          console.error(`3. If behind a proxy, set HTTP_PROXY/HTTPS_PROXY environment variables`);
          console.error(`4. For self-signed certificates, try: NODE_TLS_REJECT_UNAUTHORIZED=0 diogenes ...`);
        } else if (errorMsg.includes('api key') || errorMsg.includes('authentication') || errorMsg.includes('invalid')) {
          console.error(`\n${colors.yellow}API key troubleshooting:${colors.reset}`);
          console.error(`1. Verify your API key is correct`);
          console.error(`2. Check if the key has sufficient permissions/quota`);
          console.error(`3. Generate a new key at https://platform.openai.com/api-keys`);
        }
      },
    });
    ...
    if (result.success) {
      console.log(`${colors.green}${colors.bright}✓ Task completed successfully!${colors.reset}`);
      console.log(`${colors.dim}Iterations: ${result.iterations}, Time: ${elapsedTime}s${colors.reset}`);
...
async function main(): Promise<void> {
  ...
  if (config.security?.workspaceRoot) {
    try {
      ...
      console.error(`${colors.red}Error: Workspace directory not accessible: ${config.security.workspaceRoot}${colors.reset}`);
      process.exit(1);
    }
  }
